...
public class GeneratorFirstTest {
  ...
  @MethodSource()
  public void first_test_with_specific_amount(String language, String expected) throws Exception {
    GTE.execute(
        expected,
        language,
        p -> {
  ...
  private static Stream<Arguments> first_test_with_condition() {
    return ExpectationBuilder.newExpectation()
        .javaResult(
            "huml.FIRST(huml.WHERE(model.getAddresses(), x -> huml.EQUALS(x.getCity(), \"Berlin\")))")
        .toStream();
  }

  @ParameterizedTest(name = GTE.PARAM_TEST_NAME)
  @MethodSource()
  public void first_test_with_condition(String language, String expected) throws Exception {
    GTE.execute(
        expected,
        language,
        p -> {
  ...
  private static Stream<Arguments> first_test_with_condition_and_amount() {
    return ExpectationBuilder.newExpectation()
        .javaResult(
            "huml.FIRST(huml.WHERE(model.getAddresses(), x -> huml.EQUALS(x.getCity(), \"Berlin\")), 1.0)")
        .toStream();
  }
  ...
  private static Stream<Arguments> first_test_with_condition_group_and_amount() {
    return ExpectationBuilder.newExpectation()
        .javaResult(
            "huml.FIRST(huml.WHERE(model.getAddresses(), x -> (huml.EQUALS(x.getCity(), \"Berlin\") && huml.NOT_EQUALS(x.getCity(), x.getBirthplace()))), 1.0)")
        .toStream();
  }
