...
public class GeneratorFunctionTest {
  ...
  private static Stream<Arguments> schould_generate_function() {
    return Stream.of(
        Arguments.of(
            "java",
            "(huml.SUM_OF(huml.getArrayOf(model.getPortfolio().getShares(), s -> s.getPercentage())))")
        // Arguments.of("csharp", "huml.SUM_OF(huml.getArrayOf(model.Portfolio.Shares, s =>
        // s.Percentage))")
        // Arguments.of("javascript", "huml.SUM_OF(huml.getArrayOf(model.Portfolio.Shares, s =>
        // s.Percentage))")
        );
  }
  ...
  private static Stream<Arguments> schould_generate_function_with_lambda() {
    return Stream.of(
        Arguments.of(
            ...
            "(huml.SUM_OF(huml.getArrayOf(model.getPortfolio().getShares(), s -> s.getPercentage())))"),
        Arguments.of(
            "csharp", "(huml.SUM_OF(huml.getArrayOf(model.Portfolio.Shares, s => s.Percentage)))")
        // Arguments.of("javascript", "huml.SUM_OF(huml.getArrayOf(model.Portfolio.Shares, s =>
        // s.Percentage))")
        );
  }
