...
public class FirstTest {
  ...
  void one_parameter_applied_on_static_string() throws Exception {
    runner.run(
        ...
        r ->
            r.containsValidationMessage(
                "The function FIRST has to be applied on an array property or a nested function. Currently applied on ASTOperandStaticString"));
  }
  ...
  void one_parameter_applied_on_array_behind_variable_with_missing_left_operand() throws Exception {
    runner.run(
            "numbers AS var\n\n" +
                    "FIRST from var with greater than val AS var",
            "{numbers: [{val: 12}]}",
            r -> r.containsValidationMessage("missing left operand in condition"));
  }
}
