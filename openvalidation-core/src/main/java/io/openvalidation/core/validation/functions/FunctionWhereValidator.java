...
public class FunctionWhereValidator extends FunctionValidatorBase {
    ...
    public void validate() throws Exception {
        ...
        if (parameters.size() != 2)
            throw new ASTValidationException("Invalid number of parameters. The function "
                    + function.getName()
                    + "needs an array property/variable and a condition as a parameter", function);
        else {
            ...
            if(!(secondParam instanceof ASTOperandLambdaCondition))
            {
                throw new ASTValidationException("The second parameter of the function "
                            + function.getName()
                            + " has to be a condition. Found: " + secondParam.getClass().getSimpleName(), function);
